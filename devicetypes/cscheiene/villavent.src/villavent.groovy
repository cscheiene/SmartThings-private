 ...
 */
metadata {
	definition (name: "Villavent", namespace: "cscheiene", author: "SmartThings and gpsmith") {
	...
	tiles (scale: 2) {
    	...
    	standardTile("powername", "device.power", decoration: "flat", width: 3, height: 2) {
            ...
            state("17", label:'Low')
            state("18", label:'Low')
            state("43", label:'Normal')
            state("44", label:'Normal')
            state("45", label:'Normal')
            state("46", label:'Normal')
            state("47", label:'Normal')
            state("48", label:'Normal')
            state("49", label:'Normal')
            ...
            state("109", label:'High')
            state("110", label:'High')
            state("111", label:'High')
            state("112", label:'High')
            state("113", label:'High')
            state("114", label:'High')
            state("115", label:'High')
            state("116", label:'High')
            state("117", label:'High')
            state("118", label:'High')
            state("119", label:'High')
            state("120", label:'High')
    	}
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd) {
	if (cmd.scale == 0) {
		[name: "energy", value: (String.format("%.2f", cmd.scaledMeterValue)), unit: "kWh"]
	} else if (cmd.scale == 1) {
